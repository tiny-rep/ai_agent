...
const { loading: webAppChatLoading, handleWebAppChat, handleExLinkWebAppChat } = useWebAppChat()
...
//modify: 2025-06-29 sam 外链直接操作的信息
const exlinkSwitchStore = useExlinkSwitchInfoStore()
const { handleInitNewConversation } = useInitNewConversation()
...
// 判断是否为外链访问
const isExLink = () => {
  return ['ex-link-web-app-index', 'ex-link-inner-web-app-index'].includes(route.name as string)
}
// 判断是否为嵌外链访问
const isInnerExLink = () => {
  return ['ex-link-inner-web-app-index'].includes(route.name as string)
}

// 获取end_user_id
const getEndUserId = () => {
  return isExLink() ? String(route.params?.end_user_id) : ''
}
...
const changeIsPinned = async (idx: number, origin_is_pinned: boolean) => {
  ...
  // 3.2 调用hooks发起api请求
  await handleUpdateConversationIsPinned(conversation.id, !origin_is_pinned, getEndUserId(), () => {
    // 3.3 执行成功调用回调，更新会话位置
    if (origin_is_pinned) {
...
const deleteConversation = async (idx: number, origin_is_pinned: boolean) => {
  ...
  // 6.2 调用hooks发起请求
  handleDeleteConversation(conversation.id, getEndUserId(), () => {
    // 6.3 执行成功调用回调，删除回话
    if (origin_is_pinned) {
      pinned_conversations.value.splice(idx, 1)
    } else {
...
const handleScroll = async (event: UIEvent) => {
  ...
  if (scrollTop <= 0 && !webAppChatLoading.value) {
    saveScrollHeight()
    await loadConversationMessagesWithPage(conversation.value.id, false, getEndUserId())
...
const handleSubmit = async () => {
  ...
  if (isExLink() && web_app.value) {
    req.stream = true
    req.app_id = web_app.value.id
    req.end_user_id = String(route.params?.end_user_id)
  }
  const chatToken = isExLink() ? String(route.params.ex_link_token) : String(route.params?.token)
  const chatFun = isExLink() ? handleExLinkWebAppChat : handleWebAppChat
  await chatFun(chatToken, req, (event_response) => {
  ...
  if (messages.value.length > 0) {
    ...
    if (web_app.value?.app_config?.suggested_after_answer.enable && message_id.value) {
      await handleGenerateSUggestedQuesions(message_id.value, isExLink() ? String(route.params?.end_user_id) : '')
...
const handleStop = async () => {
  ...
  // 13.2 调用api接口中断请求
  await handleStopWebAppChat(chatToken, task_id.value, isExLink())
}
...
watch(
  ...
  async (newValue) => {
    ...
    if (newValue === 'new_conversation') {
      ...
      await loadConversationMessagesWithPage(newValue, true, getEndUserId())
      await nextTick( async () => {
        if (scroller.value) {
          scroller.value.scrollToBottom()
        }
        //modify: 2025-07-01 sam 如果是外链且在对话框有待发送的消息，自动发送
...
onMounted(async () => {
  if (isExLink()) {
    //modify: 2-25-04-14 sam  16.1 外链模式
    const ex_link_token = String(route.params?.ex_link_token)
    const end_user_id = String(route.params?.end_user_id)
    if (isInnerExLink()) {
      await loadWebApp(ex_link_token, true)
    } else {
      await Promise.all([loadWebApp(ex_link_token, true), loadWebAppConversations(ex_link_token, end_user_id, true)])
    }
  } else {
    // 16.1 提取WebApp凭证标识
    ...
    // 16.2 异步加载数据
    await Promise.all([loadWebApp(token, false), loadWebAppConversations(token, '', false)])
  }
  ...
  if (isInnerExLink()) {
    ...
    exlinkSwitchStore.$subscribe( async (mutation, state) => {
      query.value = state.exlinkSwitchInfo.question
      const end_user_id = String(route.params?.end_user_id)
      const ex_link_token = String(route.params?.ex_link_token)
      if (!state.exlinkSwitchInfo.conversationId || selectedConversation.value == state.exlinkSwitchInfo.conversationId ) {
        if (!isEmpty(query.value)) {
          await nextTick(async () => {
            await handleSubmit()
          })
        ...
        const app_id = web_app.value.id
        const conversation_id = state.exlinkSwitchInfo.conversationId
        await handleInitNewConversation(conversation_id, end_user_id, app_id)
        selectedConversation.value = conversation_id
      }
      if (!firstLoaded) {
        // 重新load对话列表
        await loadWebAppConversations(ex_link_token, end_user_id, true)
      }
      firstLoaded = true
    }, {immediate: true})
  }
...
<template>
  <div class="flex min-h-screen h-full">
    ...
    <update-name-modal
      ...
      :end_user_id="String(route.params?.end_user_id)"
