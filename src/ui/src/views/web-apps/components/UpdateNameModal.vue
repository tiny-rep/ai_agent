...
import { useUpdateConversationName, useGetConversationName } from '@/hooks/use-conversation'
...
const props = defineProps({
  ...
  end_user_id: { type: String, default: '',  required: false },
...
const saveName = async ({ errors }: { errors: Record<string, ValidatedError> | undefined }) => {
  ...
  await handleUpdateConversationName(props.conversation_id, form.value.name, props.end_user_id)
...
watch(
  ...
  async (newValue) => {
    ...
    if (!newValue) {
      ...
      await loadConversationName(props.conversation_id, props.end_user_id)
      form.value.name = conversationName.value
    }
  },
)
</script>
