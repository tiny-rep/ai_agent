...
import ToolLLMNode from './components/nodes/ToolLLMNode.vue'
...
import ToolLlmNodeInfo from './components/infos/ToolLLMNodeInfo.vue'
...
const NODE_DATA_MAP: Record<string, any> = {
  ...
  // modify: 2025-05-22 sam 新增
  tool_llm: {
    title: '工具执行',
    description: '通过大模型智能识别需要调用的系统插件、API插件和MCP工具',
    tool_type: '',
    prompt: '',
    tools: [],
    language_model_config: {
  ...
  <div class="min-h-screen flex flex-col h-full overflow-hidden relative">
    ...
    <div class="flex-1">
      ...
      >
          ...
          <div class="p-[5px] bg-white rounded-xl border z-50">
            <a-space :size="8">
              ...
              <a-trigger position="top" :popup-translate="[0, -16]">
                ...
                <template #content>
                  <div
                    class="bg-white borer border-gray-200 w-[240px] h-[720px] shadow rounded-xl overflow-auto py-2"
                  >
                    <!-- 开始节点 -->
                    <div
                    ...
                    >
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <div class="text-gray-700 font-semibold">工具执行</div>
                      </div>
                      <!-- 节点描述 -->
                      <div class="text-gray-500 text-xs">
                        通过大模型智能识别需要调用的系统插件、API插件和MCP工具。
                      </div>
                    </div>
                    <!-- 模板转换 -->
                    <div
        ...
        />
        <tool-llm-node-info
          v-if="selectedNode && selectedNode?.type === 'tool_llm'"
          :loading="updateDraftGraphLoading"
          :node="selectedNode"
          v-model:visible="nodeInfoVisible"
          @update-node="onUpdateNode"
