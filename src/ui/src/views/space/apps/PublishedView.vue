...
import { useGetPublishedConfig,
  useRegenerateWebAppToken,
  useGenerateWebAppExlinkToken,
  useCancelWebAppExlinkToken } from '@/hooks/use-app'
...
const { loading: generateWebAppExLinkTokenLoading, exLinkToken , handleGenerateWebAppExlinkToken } = useGenerateWebAppExlinkToken()
const { loading: cancelWebAppExLinkTokenLoading, handleCancelWebAppExlinkToken } = useCancelWebAppExlinkToken()
...
const exLinkWebAppUrl = computed(() => {
  if (published_config.value?.web_app?.status === 'published' && published_config.value?.web_app?.ex_link_token) {
    return getFullPath('ex-link-web-app-index', {
      'ex_link_token': published_config.value?.web_app?.ex_link_token,
      'end_user_id': 'slot'
    })
  }
  return ''
            ...
            <td class="py-3 px-4">
              <div class="flex items-center gap-3">
                ...
                <div class="flex items-center">
                  <div
                    class="bg-gray-100 h-8 leading-8 px-3 rounded-tl-lg rounded-bl-lg text-gray-700 w-[600px] max-w-[660px] line-clamp-1 break-all"
                  >
          ...
          <tr class="border-b">
            <td class="py-3 px-4 w-2/3">
              <div class="flex items-center gap-2">
                ...
                <div class="flex flex-col">
                  <div class="text-gray-700 font-semibold">网页外链</div>
                  <div class="text-gray-500">可通过访问PC网页立即开始对话（外链模式）。slot为位符，访问时必须替换为UUID格式的终端用户ID</div>
                </div>
              </div>
            </td>
            <td class="py-3 px-4 w-1/12">
              <a-tag v-if="published_config?.web_app?.status !== 'published' || !published_config.web_app.ex_link_token" color="gray" bordered>
                <template #icon>
                  <icon-minus-circle />
                </template>
                未发布
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="flex items-center">
                  <div
                    class="bg-gray-100 h-8 leading-8 px-3 rounded-tl-lg rounded-bl-lg text-gray-700 w-[600px] max-w-[660px] line-clamp-1 break-all"
                  >
                    <template v-if="published_config?.web_app?.status === 'published' && published_config.web_app.ex_link_token ">
                      {{ exLinkWebAppUrl }}
                    </template>
                    <template v-else>应用未发布成外链，无法通过外链访问</template>
                  </div>
                  <a-button
                    :loading="generateWebAppExLinkTokenLoading"
                    :disabled="published_config?.web_app?.status !== 'published'"
                    type="primary"
                    class="rounded-tr-lg rounded-br-lg px-2"
                    @click="
                      async () => {
                        // 1.调用API接口发起请求
                        await handleGenerateWebAppExlinkToken(String(route.params?.app_id))

                        published_config.web_app.ex_link_token = exLinkToken
                      }
                    "
                  >
                    生成外链
                ...
                <!-- 右侧访问按钮 -->
                <a-button class="rounded-lg px-2"
                  :loading="cancelWebAppExLinkTokenLoading"
                  :disabled="published_config?.web_app?.status !== 'published'"
                  @click="
                    async () => {
                      await handleCancelWebAppExlinkToken(String(route.params?.app_id))

                      published_config.web_app.ex_link_token = ''
                    }
                  "
                >
                  取消外链 
