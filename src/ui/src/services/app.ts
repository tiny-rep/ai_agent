...
export const getDebugConversationMessagesWithPage = (
  ...
  return get<GetDebugConversationMessagesWithPageResponse>(`/apps/${app_id}/conversations/messages`, {params: req})
}
...
//生成外链 WebAPP 的凭证标识
export const generateWebAppExlinkToken = (app_id: string) => {
  return get<GetPublishedConfigResponse>(
    `/apps/${app_id}/generate-ex-link`,
  )
}

//取消外链 WebAPP 的凭证标识
export const cancelWebAppExlinkToken = (app_id: string) => {
  return get<BaseResponse<any>>(
    `/apps/${app_id}/generate-ex-link/cancel`,
  )
}