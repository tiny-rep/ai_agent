...
class RetrievalService(BaseService):
    ...
    def search_in_dataset(self,
        ...
        """根据query+知识库列表执行检索，并返回检索的文档+得分数据（如果检索策略为全文检索，则得分为0）"""

        # 1. 提取知识库列表并校验权限同时更新知识为Id
        ...
        dataset_ids = [db.id for db in datasets]

        # 2. 构建不同种类的检索器
        from internal.core.retrievers import SemanticRetriever, FullTextRetriever
        semantic_retriever = SemanticRetriever(
            dataset_ids=dataset_ids,
            vector_store=self.vector_dataset_service.vector_store,
            search_kwargs={
                "k": k,
        ...
        # 4. 添加知识库查询记录(只存储唯一记录，也就是一个知识如果检索了多篇文档，也只存储一条)
    ...
    def create_langchain_tool_from_search(self,
                                          flask_app: Flask,
                                          dataset_ids: list[UUID],
                                          account_id: UUID,
                                          retrieval_strategy: str = RetrievalStrategy.SEMANTIC,
        ...
        def dataset_retrieval(query: str) -> str:
            """如果需要搜索扩展的知识库内容，当你没得用户的提问超过你的知识范围时，可以尝试调用此工具，输入为搜索query语句，返回数据为检索内容字符串"""
            # modify: 2025-08-29 sam 注释flask上下文激活函数，原因：在智能体执行是已经外部统一激活上下文了
            # with flask_app.app_context():
            documents = self.search_in_dataset(
                dataset_ids=dataset_ids,
                query=query,
