...
class OAuthService(BaseService):
    ...
    def get_oauth_by_provider_name(cls, provider_name: str) -> OAuth:
        ...
        if oauth is None:
            raise NotFoundException(f"授权方式{provider_name}不存在")
        return oauth

    def oatuh_login(self, provider_name: str, code: str) -> dict[str, Any]:
        ...
        # 3. 根据获取的token提取userInfo信息
        oauth_user_info = oauth.get_user_info(oauth_access_token)

        # 4. 根据provider_name + user_id(openid)获取授权记录
