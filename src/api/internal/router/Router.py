...
from internal.handler import (AppHandler, BuiltinToolHandler, ApiToolHandler,
                              ...
                              AuthHandler, AccountHandler, AIHandler, ApiKeyHandler,
                              OpenapiHandler, BuiltinAppHandler,
                              WorkflowHandler, LanguageModelHandler,
                              AssistantAgentHandler, AnalysisHandler,
                              ConversationHandler, WebAppHandler, AudioHandler, PlatformHandler, WechatHandler,
                              McpToolHandler)
...
class Router:
    ...
    def register_router(self, app: Flask):
        ...
        bp.add_url_rule(
            "/apps/<uuid:app_id>/generate-ex-link",  # 发布外链
            view_func=self.app_handler.generate_token_with_ex_link,
        )
        bp.add_url_rule(
            "/apps/<uuid:app_id>/generate-ex-link/cancel",  # 取消外链
            view_func=self.app_handler.cancel_published_with_ex_link,
        )
        ...
        bp.add_url_rule("/builtin-tools/<string:provider_name>/exec-tool/<string:tool_name>",
                        view_func=self.builtin_tool_handler.tool_exec_test)
        ...
        bp.add_url_rule("/api-tools/test", view_func=self.api_tool_handler.test_api_tool_invoke)
        ...
        bp.add_url_rule("/ai/<uuid:end_user_id>/suggested-questions/ex-link", methods=["POST"],
                        view_func=self.openapi_handler.generate_suggested_questions_in_ex_link)

        # API秘钥模块
        bp.add_url_rule("/openapi/api-keys", view_func=self.api_key_handler.get_api_keys_with_page)
        bp.add_url_rule('/openapi/api-keys',
                        methods=["POST"],
                        view_func=self.api_key_handler.create_api_key)
        bp.add_url_rule('/openapi/api-keys/<uuid:api_key_id>',
                        methods=["POST"],
                        view_func=self.api_key_handler.update_api_key)
        bp.add_url_rule('/openapi/api-keys/<uuid:api_key_id>/is-active',
                        methods=["POST"],
                        view_func=self.api_key_handler.update_api_key_is_active)
        bp.add_url_rule('/openapi/api-keys/<uuid:api_key_id>/delete',
                        methods=["POST"],
                        view_func=self.api_key_handler.delete_api_key)
        # 2025-07-01 sam 增加初始化会话接口
        bp.add_url_rule('/openapi/<uuid:end_user_id>/app/<uuid:app_id>/init-conversation/<uuid:conversation_id>',
                        methods=["GET"],
                        view_func=self.openapi_handler.init_new_conversation)

        # 2025-05-31 sam 增加开放广场接口
        bp.add_url_rule('/openapi/apps/ex-link/list',
                        view_func=self.openapi_handler.get_apps_with_page_ex_link)
        bp.add_url_rule('/openapi/apps/ex-link/hot',
                        view_func=self.openapi_handler.get_hot_apps_with_ex_link)

        openapi_bp.add_url_rule('/openapi/chat',
                                methods=["POST"],
        ...
        bp.add_url_rule('/builtin-apps/add-builtin-app-to-space',
                        methods=["POST"],
        ...
        # modify: 2025-04-11 sam 增加外链相关接口
        bp.add_url_rule("/web-apps/<string:ex_link_token>/conversations/<uuid:end_user_id>/ex-link",
                        view_func=self.web_app_handler.get_conversations_with_ex_link)
        bp.add_url_rule("/web-apps/<string:ex_link_token>/generate-access-token",
                        methods=["POST"],
                        view_func=self.web_app_handler.generate_access_token_with_ex_link)
        bp.add_url_rule("/web-apps/<string:ex_link_token>/ex-link/chat",
                        methods=["POST"],
                        view_func=self.openapi_handler.ex_link_chat)
        bp.add_url_rule("/web-apps/<string:ex_link_token>/ex-link",
                        view_func=self.web_app_handler.get_web_app_with_ex_link)
        bp.add_url_rule("/web-apps/<string:ex_link_token>/ex-link/chat/<uuid:task_id>/stop",
                        methods=["POST"],
                        view_func=self.web_app_handler.stop_web_app_chat_ex_link)
        ...
        bp.add_url_rule(
            ...
            view_func=self.openapi_handler.get_conversation_messages_with_page_ex_link,
        )
        bp.add_url_rule(
            "/conversations/<uuid:conversation_id>/delete/ex-link/<uuid:end_user_id>",
            methods=["POST"],
            view_func=self.openapi_handler.delete_conversation_ex_link,
        )
        bp.add_url_rule(
            "/conversations/<uuid:conversation_id>/messages/<uuid:message_id>/delete/ex-link/<uuid:end_user_id>",
            methods=["POST"],
            view_func=self.openapi_handler.delete_message_ex_link,
        )
        bp.add_url_rule(
            "/conversations/<uuid:conversation_id>/name/ex-link/<uuid:end_user_id>",
            view_func=self.openapi_handler.get_conversation_name_ex_link,
        )
        bp.add_url_rule(
            "/conversations/<uuid:conversation_id>/name/ex-link/<uuid:end_user_id>",
            methods=["POST"],
            view_func=self.openapi_handler.update_conversation_name_ex_link,
        )
        bp.add_url_rule(
            "/conversations/<uuid:conversation_id>/is-pinned/ex-link/<uuid:end_user_id>",
            methods=["POST"],
            view_func=self.openapi_handler.update_conversation_is_pinned_ex_link,
        )
        ...
        bp.add_url_rule(
            "/mcp-tools",
            view_func=self.mcp_tool_handler.get_mcp_tools_with_page
        )
        bp.add_url_rule(
            "/mcp-tools",
            methods=["POST"],
            view_func=self.mcp_tool_handler.create_mcp_tool
        )
        bp.add_url_rule(
            "/mcp-tools/<uuid:mcp_tool_id>",
            view_func=self.mcp_tool_handler.get_mcp_tool
        )
        bp.add_url_rule(
            "/mcp-tools/<uuid:mcp_tool_id>",
            methods=["POST"],
            view_func=self.mcp_tool_handler.update_mcp_tool
        )
        bp.add_url_rule(
            "/mcp-tools/<uuid:mcp_tool_id>/delete",
            methods=["POST"],
            view_func=self.mcp_tool_handler.delete_mcp_tool
        )
