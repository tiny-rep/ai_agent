...
class ProviderEntity(BaseModel):
    ...
    icon: str = ""  # 提供商的图标
    background: str = ""  # 提供商的图标背景颜色
    supported_model_types: list[ModelType] = Field(default_factory=list)  # 支持的模型类型


class Provider(BaseModel):
    """大语言模型服务提供商，包含提供商的所有大语言模型、描述、图标、标签等多个信息"""
    name: str  # 提供商名字
    position: int  # 提供商的位置信息
    ...
    def validate_provider(self) -> Self:
        ...
        if not isinstance(positions_yaml_data, list):
            raise FailException("positions.yaml数据格式化错误")
        ...
        for model_name in positions_yaml_data:
            ...
            for parameter in yaml_parameters:
                ...
                if use_template:
                    # 10 使用了模板，则使用模板实例剩余数据，并删除use_template
