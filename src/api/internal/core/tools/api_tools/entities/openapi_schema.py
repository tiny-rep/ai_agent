...
class OpenAPISchema(BaseModel):
    """OpenAPI规范的数据结构"""
    server: str = Field(description="工具提供者的服务基础地址", default="", validate_default=True)
    description: str = Field(description="工具提供者的描述信息", default="", validate_default=True)
    paths: dict[str, dict] = Field(description="工具提供者的路径参数字典", default_factory=dict, validate_default=True)
    ...
    def validate_paths(cls, paths: dict[str, dict]) -> dict[str, dict]:
        ...
        # 2. 提取paths里面的每一个元素，并获取元素下的get/post方法对应的值
        methods = ["get", "post"]
        interfaces = []
        extra_paths = {}
        ...
        for interface in interfaces:
            ...
            for parameter in interface["operation"].get("parameters", []):
                # 8. 检验name/in/description/required/type参数是否存在，并且类型正常
                if not isinstance(parameter.get("name"), str):
                    raise ValidateErrorException("parameter.name参数必须为字符串且不能为空")
                if not isinstance(parameter.get("description"), str):
                    raise ValidateErrorException("parameter.description参数必须为字符串且不能为空")
                if not isinstance(parameter.get("required"), bool):
                    raise ValidateErrorException("parameter.required参数必须为布尔值且不能为空")
