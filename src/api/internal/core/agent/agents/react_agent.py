...
class ReactAgent(FunctionCallAgent):
    """ReAct智能体，基于FunctionCallAgent实现的"""
    ...
    def _long_term_memory_recall_node(self, state: AgentState) -> AgentState:
        ...
        # 9. 将预设消息添加到用户消息的前面
        return {
            "messages": [RemoveMessage(human_message.id), *preset_messages]
        }
        ...
        if generation_type == "thought":
            try:
                ...
                self.agent_queue_manager.publish(state["task_id"], AgentThought(
                    ...
                    # modify: 2025-03-24 sam 从tool_calls -> content
    ...
    def _tools_node(self, state: AgentState) -> AgentState:
        """重写工具节点，处理工具工具节点的'AI工具调用参数消息'与‘工具消息接入人类消息’"""
