...
class App(db.Model):
    ...
    ex_link_token = Column(String(255), nullable=True, server_default=text("''::character varying"))  # 应用外链凭证信息
    ...
    @property
    def ex_link_token_with_default(self) -> str:
        """获取带有默认值的应用外链 token"""
        if self.status != AppStatus.PUBLISHED:
            if self.ex_link_token is not None or self.ex_link_token != "":
                self.ex_link_token = None
                db.session.commit()
        if self.ex_link_token is None or self.ex_link_token == "":
            self.ex_link_token = f"ex_link_{generate_random_string(16)}"
            db.session.commit()
        return self.ex_link_token
...
class AppConfig(db.Model):
    ...
    mcps = Column(JSONB, nullable=False, server_default=text("'[]'::jsonb"))  # 应用关联的Mcp列表
